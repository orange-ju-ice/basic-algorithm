...
if __name__ == '__main__':
    ...
    r_norm_RK = []
    r_norm_RKR = []
    x_experiment1 = x.copy()
    x_experiment2 = x.copy()

    for i in range(run_max):
        r_norm_RK.append(RK(A_nm, b, x_true, x_experiment1, iter_max, A_nm_norm))
        if i % 5 == 0:
            print(str(i))
    r_norm_RK = np.mean(np.array(r_norm_RK), axis=0)

    for i in range(run_max):
        r_norm_RKR.append(RK_Removal(A_nm, b, x_true, x_experiment2, iter_max, A_nm_norm))
        if i % 5 == 0:
            print(str(i))
    r_norm_RKR = np.mean(np.array(r_norm_RKR), axis=0)

    # Plot Figure
    plt.figure(figcont, figsize=(9, 7))
    plt.semilogy(cont, r_norm_RK,
                 linewidth='4', linestyle='-',
                 color='r', label="Random Kaczmarz")
    plt.semilogy(cont, r_norm_RKR,
                 linewidth='4', linestyle=':',
                 color='b', label="RK with Removal")
